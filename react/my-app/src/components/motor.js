...
function Motor() {
    ...
    const [dept90IsSet, dept90IsSetPointer] = useState(false)
    ...
    const {fileList: fileList} = useLoaderData();

    const [useSdfList, setUseSdfList] = useState(false)
    const [useSpectrumList, setUseSpectrumList] = useState(false)
    const [useDept135List, setUseDept135List] = useState(false)
    const [useDept90List, setUseDept90List] = useState(false)
            ...
            async function fetchData() {
                ...
                const requestOptions = {
                    ...
                    body: JSON.stringify({
                        ...
                        dept90: dept90,
                        params: params,
                        useSdfDatabase: useSdfList,
                        useSpectrumDatabase: useSpectrumList,
                        useDept135Database: useDept135List,
                        useDept90Database: useDept90List
                    })
                };
                ...
                fetch("http://localhost:9000/motor", requestOptions).then((response) => {
                    response.json().then((json) => {
                        ...
                        setUseSdfList(false);
                        setUseSpectrumList(false);
                        setUseDept135List(false);
                        setUseDept90List(false);
                    })
                }).catch((err) => {
                    console.log(err);
                }).finally(() => {
    ...
    function switchRmnMode() {
        ...
        setDEPT135(undefined)
        setDEPT90(undefined)
        setUseSpectrumList(!useSpectrumList)
    }

    function switchDept135Mode() {
        setDEPT135(undefined)
        dept135IsSetPointer(false)
        setUseDept135List(!useDept135List)
    }

    function switchDept90Mode() {
        setDEPT90(undefined)
        dept90IsSetPointer(false)
        setUseDept90List(!useDept90List)
    }
    ...
    return (
        ...
        <>
            ...
            {molecule_nbr === -1
                ...
                <>
                    {!loading
                        ? <>
                            <div className='rmn-fileLoaderList'>
                                <button className={useSdfList ? 'rmn-activeSdfButton' : 'rmn-sdfButton'}
                                        onClick={() => {
                                            switchSdfMode()
                                        }}>Use SDF file in the database
                                </button>
                                <button className={useSpectrumList ? 'rmn-activeSdfButton' : 'rmn-sdfButton'}
                                        onClick={() => {
                                            switchRmnMode()
                                        }}>Use Spectrum file in the database
                                </button>
                                <button className={useDept135List ? 'rmn-activeSdfButton' : 'rmn-sdfButton'}
                                        onClick={() => {
                                            switchDept135Mode()
                                        }}>Use Dept135 file in the database
                                </button>
                                <button className={useDept90List ? 'rmn-activeSdfButton' : 'rmn-sdfButton'}
                                        onClick={() => {
                                            switchDept90Mode()
                                        }}>Use Dept90 file in the database
                                </button>
                                <FileLoaderList
                                    fileList={fileList}
                                    useSdfList={useSdfList}
                                    useSpectrumList={useSpectrumList}
                                    useDept135List={useDept135List}
                                    useDept90List={useDept90List}
                                    setSDF={setSDF}
                                    ...
                                    dept90IsSet={dept90IsSet}
                                    dept90IsSetPointer={dept90IsSetPointer}
                    ...
                    {loading
                        ? <Loading/>
                        : <div>
                            <button className='rmn-runButton' onClick={() => {
                                check()
                            }}>
            ...
            {molecule_nbr !== -1
                ? <Molecule molecule={data.ranking[molecule_nbr]} setMolecule_nbr={setMolecule_nbr}
            ...
            {!loading && data && molecule_nbr === -1
                ...
                <>
                    <div>
                        <button className='rmn-runButton' onClick={() => {
                            Save(data, deleteData, titles)
                        }}>Save
                        </button>
                    </div>
                    <MoleculesList data={data} setData={setData} chooseMolecule={setMolecule_nbr}
